from django.test import TestCase
from django.utils.http import urlencode
from rest_framework.test import APITestCase
from rest_framework.reverse import reverse
from rest_framework import status
from .models import GameCategory, Player


# Create your tests here.
class GameCategoryTests(APITestCase):

    def create_game_category(self, name):
        url = reverse('gamecategory-list')
        data = {'name': name}
        response = self.client.post(url, data, format='json')
        return response

    def test_create_and_retrieve_game_category(self):
        """
        Ensure we can create a new GameCategory and the retrieve it
        """

        new_game_category_name = 'New Game Category'
        response = self.create_game_category(new_game_category_name)

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(GameCategory.objects.count(), 1)
        self.assertEqual(GameCategory.objects.get().name, new_game_category_name)
        print('PK {}'.format(GameCategory.objects.get().pk))

    def test_create_duplicated_game_category(self):
        """
        Ensure we can create a new GameCategory
        """

        new_game_category_name = 'New Game Category'
        response_origin = self.create_game_category(new_game_category_name)

        self.assertEqual(response_origin.status_code, status.HTTP_201_CREATED)

        response_duplicated = self.create_game_category(new_game_category_name)

        self.assertEqual(response_duplicated.status_code, status.HTTP_400_BAD_REQUEST)

    def test_retrieve_game_categories_list(self):
        """
        Ensure we can retrieve a game category
        """

        new_game_category_name = 'New Game Category'
        self.create_game_category(new_game_category_name)

        url = reverse('gamecategory-list')
        response = self.client.get(url, format='json')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 1)
        self.assertEqual(response.data['results'][0]['name'], new_game_category_name)

    def test_update_game_category(self):
        """
        Ensure we can update a single field for a game category
        """

        new_game_category = 'Initial Name'
        response = self.create_game_category(new_game_category)

        url = reverse('gamecategory-detail', {response.data['pk']})
        updated_game_category_name = 'Updated Game Category Name'
        data = {'name': updated_game_category_name}
        path_response = self.client.patch(url, data, format='json')

        self.assertEqual(path_response.status_code, status.HTTP_200_OK)
        self.assertEqual(path_response.data['name'], updated_game_category_name)

    def test_filter_game_category_by_name(self):
        """
        Ensure we can filter a game category by name
        """

        new_game_category_name_first = 'First Game Category Name'
        new_game_category_name_second = 'Second Game Category Name'
        self.create_game_category(new_game_category_name_first)
        self.create_game_category(new_game_category_name_second)

        filter_by_name = {'name': new_game_category_name_second}
        url = '{}?{}'.format(reverse('gamecategory-list'), urlencode(filter_by_name))
        response = self.client.get(url, format='json')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 1)
        self.assertEqual(response.data['results'][0]['name'], new_game_category_name_second)


class PlayerTests(TestCase):

    def create_player(self, name, gender):
        url = reverse('player-list')
        data = {'name': name, 'gender': gender}
        response = self.client.post(url, data, format='json')
        return response

    def test_create_and_retrieve_player(self):
        """
        Ensure we can create a new Player and the retrieve it
        """
        new_player_name = 'New Player'
        new_player_gender = 'M'
        response = self.create_player(new_player_name, new_player_gender)

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Player.objects.count(), 1)
        self.assertEqual(Player.objects.get().name, new_player_name)

    def test_create_duplicated_player(self):
        """
        Ensure we can create a new Player
        """
        new_player_name = 'New Player'
        new_player_gender = 'M'
        response_origin = self.create_player(new_player_name, new_player_gender)
        self.assertEqual(response_origin.status_code, status.HTTP_201_CREATED)

        response_duplicated = self.create_player(new_player_name, new_player_gender)
        self.assertEqual(response_duplicated.status_code, status.HTTP_400_BAD_REQUEST)

    def test_retrieve_players_list(self):
        """
        Ensure we can retrieve a player
        """
        new_player_name = 'New Player'
        new_player_gender = 'M'
        self.create_player(new_player_name, new_player_gender)

        url = reverse('player-list')
        response = self.client.get(url, format='json')

        self.assertEqual(response.status_codestatus.HTTP_200_OK)
        self.assertEqual(response.data['count'], 1)
        self.assertEqual(response.data['results'][0]['name'], new_player_name)
